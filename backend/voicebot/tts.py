...
Das globale TTSProcessor-Objekt ist zustandslos.
Abbruch-Steuerung (Barge-In) laeuft ueber ein asyncio.Event pro Session,
das in der VoicebotSession erzeugt wird.
"""
import asyncio
import io
import logging
...
class TTSProcessor:
    ...
    async def synthetisieren(self, text: str, voice: str = None,
                             abbruch: asyncio.Event = None) -> bytes:
        """
        ...
        Args:
            ...
            voice: Edge TTS Voice ID. Wenn None, Standard aus Config.
            abbruch: Event das bei Barge-In gesetzt wird (session-spezifisch).
        """
        ...
        try:
            audio = await self._generieren(text, voice=voice, abbruch=abbruch)
            return audio
        except Exception as e:
            log.error("TTS Fehler: %s", e)
            return self._stille(1000)

    async def _generieren(self, text: str, voice: str = None,
                          abbruch: asyncio.Event = None) -> bytes:
        """Edge-TTS Audio generieren und in 16kHz WAV konvertieren."""
        import edge_tts

        if abbruch and abbruch.is_set():
        ...
        wav_data = await loop.run_in_executor(None, self._mp3_zu_wav, mp3_data)
        return wav_data
