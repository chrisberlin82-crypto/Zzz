...
const getDashboard = async (req, res) => {
  try {
        ...
        const trendResults = await Promise.all(months.map(async (m) => {
          const [contractCount, contractValue, expenseTotal] = await Promise.all([
            ...
            Contract.findAll({
              ...
              attributes: [
                [sequelize.fn('COALESCE', sequelize.fn('SUM', sequelize.col('estimated_value')), 0), 'total']
              ],
              raw: true
            }),
            Expense.findAll({
              where: {
                ...
                expense_date: {
                  [Op.between]: [
                    m.start.toISOString().split('T')[0],
                    m.end.toISOString().split('T')[0]
                  ]
                }
              },
              attributes: [
    ...
    // Admin/Standortleitung: System-Metriken + Anbieter-Aufschluesselung
    ...
    if (['ADMIN', 'STANDORTLEITUNG'].includes(role)) {
      ...
      // Aufschluesselung nach Produkt/Anbieter
      ...
      providerBreakdown = productBreakdown.map(p => ({
    ...
    // Team-Performance fuer Teamleiter+
    ...
    if (['ADMIN', 'STANDORTLEITUNG', 'TEAMLEAD'].includes(role)) {
      ...
      if (memberIds && memberIds.length > 1) {
        ...
        teamPerformance = perfData.map(p => ({
